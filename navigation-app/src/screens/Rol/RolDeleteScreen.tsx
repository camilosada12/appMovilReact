...
type DetailsRouteProp = RouteProp<RolTasckParamsList, "RolDelete">;
type NavigationProp = NativeStackNavigationProp<RolTasckParamsList>;

export default function RolDeleteScreen() {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await deleteEntity(Number(id), "Rol", "Logical");
              Alert.alert("Éxito", "Rol eliminado correctamente.", [
                { text: "OK", onPress: () => navigation.goBack() },
              ]);
            } catch (error) {
              Alert.alert("Error", "Hubo un problema al eliminar el Rol.");
            }
          },
        },
      ]
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>¿Seguro que deseas eliminar este Rol?</Text>
      <Text style={styles.label}>Nombre:</Text>
      <Text style={styles.value}>{Rol.name}</Text>

      <Text style={styles.label}>Descripción:</Text>
      <Text style={styles.value}>{Rol.description}</Text>
